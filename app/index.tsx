...
/* ─── Quick-Add Transaction modal (minimal) ──────────────────────── */
const QuickAddModal = ({
  visible, onClose, db, onSave, colors, catColors,
}: {
  visible: boolean;
  onClose: () => void;
  db: any;
  ...
  const canSave = amount && subCategory;
  const color = catColors[category];

  const handleSave = () => {
    if (!canSave) return;
    onSave({ date: todayStr(), amount: parseFloat(amount) || 0, category, subCategory, description });
    onClose();
  };
...
const makeModalStyles = (colors: any) =>
  StyleSheet.create({
    ...
    cancelBtn: {
      flex: 1, padding: 14, borderRadius: 9,
...
/* ─── Stat card ─────────────────────────────────────────────────── */
const StatCard = ({ label, value, sub, color, colors }: any) => (
  <View style={[dStyles.statCard, { backgroundColor: colors.card, borderColor: colors.border, borderLeftColor: color, borderLeftWidth: 3 }]}>
    ...
    {sub ? <Text style={[dStyles.statSub, { color: colors.textDim }]}>{sub}</Text> : null}
  </View>
);

/* ─── Budget ring (simple progress bar) ──────────────────────────── */
const BudgetRow = ({ label, color, pct, real, budget, fmt, colors }: any) => {
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      ...
      <Text style={{ fontSize: 10, color: over ? colors.negative : colors.textDim, marginTop: 2, textAlign: 'right' }}>{pct}% used</Text>
    </View>
  );
};
...
const makeScreenStyles = (colors: any) => StyleSheet.create({
  ...
  banner: {
    borderWidth: 1, borderRadius: 10, padding: 18, alignItems: 'center',
  },
...
// Alias for StyleSheet at module level (used by StatCard)
